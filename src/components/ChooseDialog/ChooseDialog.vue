  ...
  <el-dialog :visible="visible" @close="handleClose" :modal-append-to-body="true" :append-to-body="true"
    ...
    <div class="container">
      <div class="top-btn">
        ...
        <el-button type="primary" :disabled="btnDisable" @click="handleChoose">选择</el-button>
      </div>
...
import { folderTree } from '@/api/api'
...
export default {
  ...
  methods: {
    ...
    selectable (row) {
      return this.filterType ? row.nodeType === 'dam:Asset' && row.metadata && this.filterType.includes(row.metadata['dc:format']) : row.nodeType === 'dam:Asset'
    },
    ...
    handleChoose () {
      const [{ path }] = this.$refs.table.selection
